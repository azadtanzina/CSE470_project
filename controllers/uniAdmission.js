  ...
  db.query(
    ...
    async (error, results) => {
      ...
      for (var i = 0; i < results.length; i++) {
        // console.log("Enteted for condition");
        console.log(level.trim() + "=====" + results[i].level.trim());
        console.log(area.trim() + "=====" + results[i].area.trim());
        if (
          level.trim() === results[i].level.trim() &&
          area.trim() === results[i].area.trim()
        ) {
          //   console.log("Enteted if condition");
  ...
  db.query(
    ...
    async (error, results) => {
      ...
      for (var i = 0; i < results.length; i++) {
        if (
          level.trim() === results[i].level.trim() &&
          id.toString().trim() === results[i].subject_id.toString().trim()
        ) {
